...
def compute_distances_two_loops(x_train, x_test):
  ...
  dists = x_train.new_zeros(num_train, num_test).double()
  ##############################################################################
  ...
  X_train = x_train.view(num_train,-1).clone() 
  X_test = x_test.view(num_test,-1).clone()
  for i, train in enumerate(X_train):
      for j, test in enumerate(X_test):
          dists[i,j] = ((train - test)**2).sum().double().sqrt()
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def compute_distances_one_loop(x_train, x_test):
  ...
  X_test = x_test.view(num_test,-1).clone()
  for i, train in enumerate(X_train):
      dists[i] =  ((train - X_test)**2).sum(1).double().sqrt()
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def compute_distances_no_loops(x_train, x_test):
  ...
  # sqrt(sum((x-y)^2)) = sqrt(sum(x^2 + y^2 - 2xy))
  # sqrt(sum((x-y)^2)) = sqrt(sum(x^2) + sum(y^2) - sum(2xy))
  test_sum =  (X_test**2).sum(1)  # sum(x^2) 
  train_sum = (X_train**2).sum(1) # sum(y^2)
  inner_product = torch.matmul(X_test, X_train.t()) # sum(xy) 
  dists = (-2 * inner_product + test_sum.reshape(-1, 1) + train_sum).double().sqrt().t() # broadcast
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def predict_labels(dists, y_train, k=1):
  ...
  for i, dist in enumerate(dists.t()):
      pred = y_train[torch.topk(dist, k, largest = False)[1]]
      pred, idx = pred.unique(return_counts=True)
      idx_max = torch.argmax(idx)
      pred = pred[idx_max]
      y_pred[i] = pred
...
def knn_cross_validate(x_train, y_train, num_folds=5, k_choices=None):
  ...
  for k in k_choices:
       ...
       for fold in range(num_folds):
           ...
           #Classification and Checking accuracies                          
           classifier_train = KnnClassifier(x_train, y_train)
           k_to_accuracies[k].append(classifier_train.check_accuracy(x_val, y_val, k=k, quiet=True))
